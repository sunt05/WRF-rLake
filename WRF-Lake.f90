...
SUBROUTINE ShalLakeFluxes(forc_t,forc_pbot,forc_psrf,forc_hgt,forc_hgt_q,           &  !i
  ...
  ITERATION : DO WHILE (iter <= niters .AND. fncopy > 0)
    ...
    ! IF (iter == 1)  ustar=0.06_r8 !initial ustar
    ...
    ! fl = (25._r8*lakedepth(p)*grav/ur(p)**2._r8)**(1._r8/3._r8)/100._r8
    ...
    !   z0hg(p) = z0mg(p)*exp(-0.13_r8*R0**(-3.2_r8))
    !   z0qg(p) = z0mg(p)*exp(-0.13_r8*R0**(-4.2_r8))
    ! ELSE IF (iter == 1) THEN
